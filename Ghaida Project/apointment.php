...
<center>
<div style="width:70%">
<button class="registerbtn" onclick="document.getElementById('id02').style.display='block'">حذف موعد</button>
<button class="registerbtn" onclick="document.getElementById('id01').style.display='block'">إضافة موعد</button>
...
<div id="id01" class="modal">
  ...
  <form class="modal-content animate" action="addapointment.php" dir="rtl" method="post">
...
if ($conn->connect_error) {
  ...
  $sql = "SELECT * FROM apointments a LEFT JOIN doctors d on a.did = d.did WHERE a.pid = '".$_SESSION['userid']."' ORDER BY a.date DESC" ;
  $result = $conn->query($sql);
  echo"<table style='width:100%'><tr><td style='width:20%'>التاريخ</td><td style='width:20%'>الوقت</td><td style='width:20%'>الطبيب</td><td style='width:20%'>الحالة</td><td></td></tr>";

  if ($result->num_rows > 0){  
  while($row = $result->fetch_assoc()) { 
  echo"<tr><tr><td>".$row['date']."</td><td>".$row['time']."</td><td>د. ".$row['f_name']."&nbsp;".$row['l_name']."</td><td>".$row['status']."</td>
